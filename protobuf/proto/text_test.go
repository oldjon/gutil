...
func anyEqual(got, want proto.Message) bool {
	// if messages are protocol.Equal, no need to marshal.
...
func TestAmbiguousAny(t *testing.T) {
	...
	`, pb)
	if err != nil {
		t.Errorf("unexpected protocol.UnmarshalText error: %v", err)
	}
}
...
func TestMarshalText(t *testing.T) {
	...
	if err := proto.MarshalText(buf, newTestMessage()); err != nil {
		t.Fatalf("protocol.MarshalText: %v", err)
	}
	got := buf.String()
	...
	for _, tc := range testCases {
		t.Run("", func(t *testing.T) {
			...
			if err := proto.MarshalText(&buf, tc.in); err != nil {
				t.Fatalf("protocol.MarsalText error: %v", err)
			}
			got := buf.String()
			if got != tc.out {
				t.Fatalf("want:\n%s\n\nwant:\n%s", got, tc.out)
			...
			if err := proto.UnmarshalText(got, pb); err != nil {
				t.Fatalf("protocol.UnmarshalText error: %v", err)
			}
			if !proto.Equal(pb, tc.in) {
				t.Fatalf("protocol.Equal mismatch:\ngot:\n%v\n\nwant:\n%v", pb, tc.in)
			}
		})
	}
}
...
var outOfSpace = errors.New("protocol: insufficient space")
...
func TestRacyMarshal(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("protocol.Marshal error: %v", err)
	}
	...
	for i := 0; i < 10; i++ {
		go func() {
			...
			if got != wantText {
				t.Errorf("protocol.MarshalTextString = %q, want %q", got, wantText)
			}
		}()
		go func() {
			defer wg.Done()
			...
			if !bytes.Equal(got, wantBytes) || err != nil {
				t.Errorf("protocol.Marshal = (%x, %v), want (%x, nil)", got, err, wantBytes)
			}
		}()
	}
}
	...
	for _, test := range unmarshalTextTests {
		t.Run("", func(t *testing.T) {
			...
			if test.err == "" {
				...
				if err != nil {
					t.Errorf("protocol.UnmarshalText error: %v", err)
				} else if !proto.Equal(pb, test.out) {
					t.Errorf("protocol.Equal mismatch:\ngot:  %v\nwant: %v", pb, test.out)
				}
			} else {
				// We do expect failure.
				if err == nil {
					t.Errorf("protocol.UnmarshalText: got nil error, want %v", test.err)
				} else if !strings.Contains(err.Error(), test.err) {
					t.Errorf("protocol.UnmarshalText error mismatch:\ngot:  %v\nwant: %v", err.Error(), test.err)
...
func TestUnmarshalTextCustomMessage(t *testing.T) {
	...
	if err := proto.UnmarshalText("not custom", msg); err == nil {
		t.Errorf("protocol.UnmarshalText: got nil error, want non-nil")
	}
}
...
func TestOneofParsing(t *testing.T) {
	...
	if err := proto.UnmarshalText(inOverwrite, m); err == nil {
		t.Errorf("protocol.UnmarshalText: got nil error, want %v", testErr)
