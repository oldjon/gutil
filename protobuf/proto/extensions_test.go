...
func TestExtensionDescsWithUnregisteredExtensions(t *testing.T) {
	...
	if descs, err := proto.ExtensionDescs(msg); len(descs) != 0 || err != nil {
		t.Errorf("protocol.ExtensionDescs: got %d descs, error %v; want 0, nil", len(descs), err)
	}
	...
	if err != nil {
		t.Fatalf("protocol.ExtensionDescs: got error %v", err)
	}
	sortExtDescs(descs)
	wantDescs := []*proto.ExtensionDesc{extdesc1, {Field: extdesc2.Field}}
	if !reflect.DeepEqual(descs, wantDescs) {
		t.Errorf("protocol.ExtensionDescs(msg) sorted extension ids: got %+v, want %+v", descs, wantDescs)
	}
}
...
func TestNilExtension(t *testing.T) {
	...
	} else if want := fmt.Sprintf("protocol: SetExtension called with nil value of type %T", new(pb2.Ext)); err.Error() != want {
...
func TestMarshalUnmarshalRepeatedExtension(t *testing.T) {
	...
	for _, test := range tests {
		...
		// Unmarshal and read the merged protocol.
...
func TestUnmarshalRepeatingNonRepeatedExtension(t *testing.T) {
	...
	// format. For example, the protocol compiler may encode custom options in
	...
	for _, test := range tests {
		...
		for i, e := range test.ext {
			// Merge to create the wanted protocol.
			proto.Merge(&want, e)
...
func TestClearAllExtensions(t *testing.T) {
	...
	if proto.HasExtension(m, desc) {
		t.Errorf("protocol.HasExtension(%s): got true, want false", proto.MarshalTextString(m))
	}
	if err := proto.SetExtension(m, desc, proto.Bool(true)); err != nil {
		t.Errorf("protocol.SetExtension(m, desc, true): got error %q, want nil", err)
	}
	if !proto.HasExtension(m, desc) {
		t.Errorf("protocol.HasExtension(%s): got false, want true", proto.MarshalTextString(m))
	}
...
func TestMarshalRace(t *testing.T) {
	...
	if err := proto.SetExtension(m, pb2.E_Ext_More, ext); err != nil {
		t.Fatalf("protocol.SetExtension(m, desc, true): got error %q, want nil", err)
	}
