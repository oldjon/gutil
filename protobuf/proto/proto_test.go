...
// When hooks are enabled, RequiredNotSetError is typed alias to internal/protocol
...
// Simple tests for protocol messages that implement the Marshaler interface.
...
func TestRequiredFieldEnforcement(t *testing.T) {
	...
	// A slightly sneaky, yet valid, protocol. It encodes the same required field twice,
...
func TestMaximumTagNumber(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("protocol.Marshal failed: %v", err)
	}
	m2 := new(pb2.MaxTag)
	if err := proto.Unmarshal(buf, m2); err != nil {
		t.Fatalf("protocol.Unmarshal failed: %v", err)
	}
...
func TestProto3ZeroValues(t *testing.T) {
	...
	for _, test := range tests {
		b, err := proto.Marshal(test.m)
		if err != nil {
			t.Errorf("%s: protocol.Marshal: %v", test.desc, err)
			continue
		}
		if len(b) > 0 {
...
func TestRoundTripProto3(t *testing.T) {
	...
	m2 := new(pb3.Message)
	if err := proto.Unmarshal(b, m2); err != nil {
		t.Fatalf("protocol.Unmarshal: %v", err)
	}
	t.Logf("m2: %v", m2)

	if !proto.Equal(m, m2) {
		t.Errorf("protocol.Equal returned false:\n m: %v\nm2: %v", m, m2)
	}
}
...
func TestMap(t *testing.T) {
	...
	var m pb3.Message
	if err := proto.Unmarshal(b, &m); err != nil {
		t.Fatalf("protocol.Unmarshal error: %v", err)
	}
...
func init() {
	if err := proto.SetExtension(messageWithExtension1, pb2.E_Ext_More, &pb2.Ext{Data: proto.String("Abbott")}); err != nil {
		log.Panicf("protocol.SetExtension: %v", err)
	}
