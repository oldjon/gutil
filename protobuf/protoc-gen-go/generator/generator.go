...
// protocol package is introduced; the generated code references
// a constant, protocol.ProtoPackageIsVersionN (where N is generatedCodeVersion).
...
// FileDescriptor describes an protocol buffer descriptor file (.protocol).
...
func (d *FileDescriptor) goFileName(pathType pathType) string {
	name := *d.Name
	if ext := path.Ext(name); ext == ".protocol" || ext == ".protodevel" {
...
type getterSymbol struct {
	name     string
	typ      string
	typeName string // canonical name in protocol world; empty for protocol.Message and similar
...
type Generator struct {
	*bytes.Buffer
	...
	ImportMap         map[string]string // Mapping from .protocol file name to import path
...
func (g *Generator) WrapTypes() {
	...
	for _, f := range g.Request.ProtoFile {
		...
		if substitution, ok := g.ImportMap[f.GetName()]; ok {
			// Command-line: M=foo.protocol=quux/bar.
			//
...
func (g *Generator) BuildTypeNameMap() {
	...
	for _, f := range g.allFiles {
		// The names in this loop are defined by the protocol world, not us, so the
...
// AnnotatedAtoms is a list of atoms (as consumed by P) that records the file name and protocol AST path from which they originated.
...
// Annotate records the file name and protocol AST path of a list of atoms
		...
		case *AnnotatedAtoms:
			...
			if g.annotateCode {
				...
				for _, token := range strings.Split(v.path, ",") {
					...
					if err != nil {
						g.Fail("could not parse protocol AST path: ", err.Error())
					}
...
func (g *Generator) generate(file *FileDescriptor) {
	...
	g.P("// is compatible with the protocol package it is being compiled against.")
	g.P("// A compilation error at this line likely means your copy of the")
	g.P("// protocol package needs to be updated.")
	g.P("const _ = ", g.Pkg["protocol"], ".ProtoPackageIsVersion", generatedCodeVersion, " // please upgrade the protocol package")
	g.P()
...
// PrintComments prints any comments from the source .protocol file.
// The path is a comma-separated list of integers.
// It returns an indication of whether any comments were printed.
// See descriptor.protocol for its format.
...
func (g *Generator) generateImports() {
	...
	// We almost always need a protocol import.  Rather than computing when we
	// do, which is tricky when there's a plugin, just import it and
	// reference it later. The same argument applies to the fmt and math packages.
	g.P("import (")
	g.P(g.Pkg["fmt"] + ` "fmt"`)
	g.P(g.Pkg["math"] + ` "math"`)
	g.P(g.Pkg["protocol"]+" ", GoImportPath(g.ImportPrefix)+"github.com/golang/protobuf/protocol")
	...
	g.P("var _ = ", g.Pkg["protocol"], ".Marshal")
...
func (g *Generator) generateEnum(enum *EnumDescriptor) {
	...
	g.P("return ", g.Pkg["protocol"], ".EnumName(", ccTypeName, "_name, int32(x))")
	g.P("}")
	g.P()

	if !enum.proto3() {
		g.P("func (x *", ccTypeName, ") UnmarshalJSON(data []byte) error {")
		g.P("value, err := ", g.Pkg["protocol"], ".UnmarshalJSONEnum(", ccTypeName, `_value, data, "`, ccTypeName, `")`)
		g.P("if err != nil {")
		g.P("return err")
		g.P("}")
...
func (g *Generator) goTag(message *Descriptor, field *descriptor.FieldDescriptorProto, wiretype string) string {
	...
	if *field.Type == descriptor.FieldDescriptorProto_TYPE_ENUM {
		...
		// original (protocol-world) package name.
		obj := g.ObjectNamed(field.GetTypeName())
		if id, ok := obj.(*ImportedDescriptor); ok {
			obj = id.o
		}
...
type fieldCommon struct {
	...
	protoName  string // Name of field in protocol language, e.g. "field_name" or "descriptor"
...
// getProtoName gets the protocol name of a field, e.g. "field_name" or "descriptor".
...
type simpleField struct {
	...
	protoDef      string                               // Default value as defined in the protocol file, e.g "yoshi" or "5"
...
// getProtoDef returns the default value explicitly stated in the protocol file, e.g "yoshi" or "5".
...
type oneofSubField struct {
	...
	fieldNumber   int                                  // Actual field number, as defined in protocol, e.g. 12
	getterDef     string                               // Default for getters, e.g. "nil", `""` or "Default_MessageType_FieldName"
	protoDef      string                               // Default value as defined in the protocol file, e.g "yoshi" or "5"
...
type defField interface {
	getProtoDef() string                                // default value explicitly stated in the protocol file, e.g "yoshi" or "5"
	getProtoName() string                               // protocol name of a field, e.g. "field_name" or "descriptor"
...
func (g *Generator) generateInternalStructFields(mc *msgCtx, topLevelFields []topLevelField) {
	...
	if len(mc.message.ExtensionRange) > 0 {
		...
		g.P(g.Pkg["protocol"], ".XXX_InternalExtensions `", messageset, "json:\"-\"`")
	}
...
func (g *Generator) generateOneofFuncs(mc *msgCtx, topLevelFields []topLevelField) {
	...
	// OneofFuncs
	g.P("// XXX_OneofWrappers is for the internal use of the protocol package.")
...
func (g *Generator) generateCommonMethods(mc *msgCtx) {
	...
	g.P("func (m *", mc.goName, ") String() string { return ", g.Pkg["protocol"], ".CompactTextString(m) }")
	...
	// if we change protocol.MessageName to work with multiple equivalents.
	...
	if len(mc.message.ExtensionRange) > 0 {
		g.P()
		g.P("var extRange_", mc.goName, " = []", g.Pkg["protocol"], ".ExtensionRange{")
		for _, r := range mc.message.ExtensionRange {
			end := fmt.Sprint(*r.End - 1) // make range inclusive on both ends
			g.P("{Start: ", r.Start, ", End: ", end, "},")
		}
		g.P("}")
		g.P("func (*", mc.goName, ") ExtensionRangeArray() []", g.Pkg["protocol"], ".ExtensionRange {")
	...
	g.P("func (m *", mc.goName, ") XXX_Merge(src ", g.Pkg["protocol"], ".Message) {")
	...
	g.P("var xxx_messageInfo_", mc.goName, " ", g.Pkg["protocol"], ".InternalMessageInfo")
	g.P()
}
...
func (g *Generator) generateMessage(message *Descriptor) {
	...
	for i, field := range message.Field {
		...
		// in the protocol file, meaning that a change in the field
	...
	g.addInitf("%s.RegisterType((*%s)(nil), %q)", g.Pkg["protocol"], goTypeName, fullName)
	// Register types for native map types.
	for _, k := range mapFieldKeys(mapFieldTypes) {
		fullName := strings.TrimPrefix(*k.TypeName, ".")
		g.addInitf("%s.RegisterMapType((%s)(nil), %q)", g.Pkg["protocol"], mapFieldTypes[k], fullName)
	}
...
func (g *Generator) generateExtension(ext *ExtensionDescriptor) {
	...
	// For text formatting, the package must be exactly what the .protocol file declares,
	...
	g.P("var ", ccTypeName, " = &", g.Pkg["protocol"], ".ExtensionDesc{")
	...
	g.addInitf("%s.RegisterExtension(%s)", g.Pkg["protocol"], ext.DescName())
...
func (g *Generator) generateFileDescriptor(file *FileDescriptor) {
	...
	v := file.VarName()
	g.P()
	g.P("func init() { ", g.Pkg["protocol"], ".RegisterFile(", strconv.Quote(*file.Name), ", ", v, ") }")
	g.P("var ", v, " = []byte{")
...
func (g *Generator) generateEnumRegistration(enum *EnumDescriptor) {
	// // We always print the full (protocol-world) package name here.
	...
	// The full type name, CamelCased.
	ccTypeName := CamelCaseSlice(typeName)
	g.addInitf("%s.RegisterEnum(%q, %[3]s_name, %[3]s_value)", g.Pkg["protocol"], pkg+ccTypeName, ccTypeName)
}
...
// which can be dotted in the input .protocol file.  It replaces non-identifier characters such as
...
// .protocol file by way of a "path", which is a sequence of integers that
// describe the route from a FileDescriptorProto to the relevant submessage.
// The path alternates between a field number of a repeated field, and an index
// into that repeated field. The constants below define the field numbers that
// are used.
//
// See descriptor.protocol for more information about this.
const (
